...
def mutate_individual(individual: list[int], crop_ids: list[int], mutation_rate: float)  -> tuple[list[int]]:
...
# === Diversity helper ===
def diversity_index_discrete(pop) -> float:
    """
    pop: iterable από Individuals (κάθε άτομο είναι λίστα int)
    Επιστρέφει δείκτη ποικιλίας D στο [0,1]
    """
    pop = list(pop)
    if not pop:
        return 0.0
    ...
    for col in cols:
        cnt = Counter(col)
        psq = sum((c / N) ** 2 for c in cnt.values())
        Ds.append(1.0 - psq)
...
def run_ga_deap(
    ...
    toolbox.register("mutate", mutate_individual, crop_ids=crop_ids, mutation_rate=mutation_rate)
    ...
    pop = toolbox.population(n=poplupation_size)
    ...
    logbook.header = ["gen", "avg", "min", "max", "var", "std", "best", "diversity"]
    ...
    D0 = diversity_index_discrete(pop)
    gens_diversity.append(D0)
    logbook.record(gen=0, **record, std=std_val, best=hof[0].fitness.values[0], diversity=D0)

    # Early stopping variables
    earlystop_std = False
    earlystop_gens = False
    ...
    for gen in range(1, generations + 1):
        ...
        # elif elitism == 1:
        #     # μ,λ
        #     pop[:] = tools.selBest(offspring, poplupation_size)
        # else:
        #     # μ+λ
        elites = tools.selBest(pop, elite_size) 
        rest = tools.selBest(offspring, poplupation_size - elite_size)
        pop[:] = elites + rest
        ...
        record = stats.compile(pop)
        std_val = np.sqrt(record['var'])
        D = diversity_index_discrete(pop)
        gens_diversity.append(D)
        logbook.record(gen=gen, **record, std=std_val, best=hof[0].fitness.values[0], diversity=D)
    ...
    # diversity tail stats
    tail_w = min(30, len(gens_diversity)) or 1
    diversity_tail = float(np.mean(gens_diversity[-tail_w:]))
    has_diversity = diversity_tail >= 0.25  # threshold για L≈8
    ...
    return (
        ...
        variance,
        worst_fitness, 
        diversity_tail,   # μέση ποικιλία στην ουρά
        has_diversity,     # True/False
        earlystop_gens,    # True/False
        earlystop_std,        # True/False
        gen
    )
...
#         # μ+λ elitism 
#         elites = tools.selBest(pop, ELITE_SIZE) 
#         rest = tools.selBest(offspring, POPULATION_SIZE - ELITE_SIZE)
#         pop[:] = elites + rest
#         hof.update(pop)
